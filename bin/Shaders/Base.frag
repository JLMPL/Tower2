...
uniform samplerCube uPointShadowCubemap[2];
...
float pointShadow0()
{
    ...
    for (int i = -1; i < 2; i++)
    {
        for (int j = -1; j < 2; j++)
        {
            for (int k = -1; k < 2; k++)
            {
                vec3 nusample = normalize(sample + vec3(i,j,k) * 0.01);
                acc += (dist < textureCube(uPointShadowCubemap[0], nusample).r + 0.01) ? 1.f : 0.f;
            }
        }
    }
    ...
    return (dist < textureCube(uPointShadowCubemap[0], sample).r + 0.01) ? 1.f : 0.1f;
}

float pointShadow1()
{
    vec3 diff = fragPos - uPointLights[1].pos;
    float dist = length(diff);

    vec3 sample = normalize(diff);

    return (dist < textureCube(uPointShadowCubemap[1], sample).r + 0.01) ? 1.f : 0.1f;
}
...
vec3 calcPointLight(PointLight pl, vec3 normal, vec3 color, vec3 toCam)
{
    ...
    vec3 attenuation = (pl.color * pl.intensity) / pow(length(tmp), 2);
...
void main()
{
    ...
    vec3 final = texture * vec3(0.001);

    final += calcPointLight(uPointLights[0], normal, texture, toCam) * pointShadow0();
    final += calcPointLight(uPointLights[1], normal, texture, toCam) * pointShadow1();
